...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || t('register.registerFailedDefault'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 p-8">
          {error && (
            <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-xl flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-red-800">{t('register.registerFailed')}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('register.username')}</label>
