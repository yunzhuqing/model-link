...
# ── Module-level singleton (sync — deprecated, kept for transition) ────────────
...
def init_cache() -> CacheService:
    """Initialise the global CacheService singleton (sync — deprecated)."""
    global _cache
    ...
    if backend_type == "redis":
        ...
        backend = RedisCacheBackend(redis_url=redis_url, default_ttl=default_ttl, key_prefix=key_prefix)
...
def get_cache() -> CacheService:
    """Return the global CacheService singleton (sync — deprecated)."""
    global _cache
    if _cache is None:
        with _cache_lock:
            if _cache is None:
...
class AsyncRedisCacheBackend(AsyncCacheBackend):
    ...
    async def incr_float_scalar(self, key: str, amount: float) -> Optional[float]:
        rk = self._key(key)
        if not await self.client.exists(rk):
            return None
        result = await self.client.incrbyfloat(rk, amount)
        return float(result)
