...
AGG_FIELDS = [
    ...
    ("total_web_search_requests", func.coalesce(func.sum(UsageRecord.web_search_requests), 0)),
    ("total_credits",           func.coalesce(func.sum(UsageRecord.credits), 0)),
]

_INT_FIELDS = ('input_tokens', 'output_tokens', 'reasoning_tokens',
               'total_image_count', 'total_video_count', 'total_web_search_requests')


def get_active_key_hashes(session: Session, since: datetime | None = None) -> dict[str, int]:
    """Return mapping of api_key_hash → max(UsageRecord.id) for keys with usage records.

    When *since* is provided, only records created after that time are considered.
    """
    q = session.query(
    ...
    rows = q.group_by(UsageRecord.api_key_hash).all()
...
def apply_delta_to_apikey(ak, delta: dict):
    ...
    ak.total_input_tokens = (ak.total_input_tokens or 0) + delta['input_tokens']
    ak.total_output_tokens = (ak.total_output_tokens or 0) + delta['output_tokens']
    ak.total_reasoning_tokens = (ak.total_reasoning_tokens or 0) + delta['reasoning_tokens']
    ...
    ak.total_web_search_requests = (ak.total_web_search_requests or 0) + delta['total_web_search_requests']
    ak.total_credits = round((ak.total_credits or 0) + float(delta['total_credits'] or 0), 6)
...
def apply_delta_to_cache(cache, key: str, ak):
    ...
    cached['total_web_search_requests'] = ak.total_web_search_requests
    cached['total_credits'] = ak.total_credits
