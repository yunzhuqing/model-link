...
# ---------------------------------------------------------------------------
# Bailian-hosted Vidu image generation tables  —  WxH → (aspect_ratio, tier)
# ---------------------------------------------------------------------------
#
# 阿里云百炼托管的 Vidu 图像生成模型仅支持固定尺寸集合（按模型区分，
# 分 1K / 2K / 4K 档）。用户侧通常以 "WxH" 传入，上游要求 "W*H"，
# 由 bailian.image_generation._resolve_bailian_vidu_size() 归一化并校验。
#
# aspect_ratio 使用最接近的标准比例标注（用于 tier + ratio 解析路径）；
# 直接传像素尺寸时以精确 WxH 匹配为准。
BAILIAN_VIDU_IMAGE_SIZE_MAP: Dict[str, SizeTable] = {
    # ── Vidu Image ──────────────────────────────────────────────────────
