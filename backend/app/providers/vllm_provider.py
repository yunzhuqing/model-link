...
class VLLMProvider(OpenAIProvider):
    ...
    async def rerank(self, request: RerankRequest) -> RerankResponse:
        ...
        vLLM 兼容 /v1/rerank 接口，格式与 RerankRequest/RerankResponse 一致。
        ...
        if request.is_multimodal:
            raise ValueError("vLLM rerank does not support multimodal inputs")
        ...
        try:
            ...
            if response.status_code >= 400:
                try:
                    ...
                    raise RuntimeError(
                        f"vLLM rerank API error ({response.status_code}): "
                        f"{json.dumps(error_data, ensure_ascii=False)}"
                    )
                except json.JSONDecodeError:
                    raise RuntimeError(
                        f"vLLM rerank API error ({response.status_code}): {response.text}"
                    )

            data = response.json()
            return self._parse_rerank_response(data, request.model, request.return_documents)

        except (RuntimeError, ValueError):
            raise
        except Exception as e:
            raise RuntimeError(f"vLLM rerank API error: {str(e)}")

    def _parse_rerank_response(
        self, data: Dict[str, Any], model: str, return_documents: bool
    ) -> RerankResponse:
        """解析 vLLM rerank 响应。"""
        ...
        usage = RerankUsage(total_tokens=usage_data.get("total_tokens", 0))
