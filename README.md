...
### 后端
- **Python 3.12+**
- **Quart** - 异步 ASGI Web 框架 (Flask 兼容 API)
- **SQLAlchemy 2.0** - 异步 ORM
- **ARQ** - 基于 Redis 的异步任务队列
- **Redis** - 缓存、限流、分布式协调、任务队列
- **Pydantic** - 数据验证
- **httpx** - 异步 HTTP 客户端
- **JWT** - 身份认证
- **Passlib** - 密码哈希
...
AI Gateway 使用 [ARQ](https://github.com/python-arq/arq) 作为异步任务队列，将高频的数据库写入操作（如 API Key 的 `last_used_at` 和 `request_count` 更新）从请求路径中卸掉，通过 Redis 队列异步消费，减少数据库压力。

### 去重机制

同一 API Key 在 5 秒窗口内的多次请求会被合并为一次数据库更新：

```
请求1 (t=0s) ──→ enqueue job (job_id=apikey_usage:42:bucket_0, defer=5s)
请求2 (t=2s) ──→ 发现同一 job_id 已存在 → 跳过（no-op）
请求3 (t=4s) ──→ 发现同一 job_id 已存在 → 跳过（no-op）
                                      ↓
                              t=5s: Worker 执行 DB UPDATE（仅一次）
```
...
服务启动时自动在 Quart 进程内启动后台 ARQ Worker，无需额外配置。如需禁用：

```bash
export ARQ_EMBEDDED_WORKER=false
```

多实例部署时，每个实例都能独立生产和消费消息，一个实例产生的任务可以被任意实例消费。
...
| 变量名 | 说明 | 默认值 |
|--------|------|--------|
| `ARQ_EMBEDDED_WORKER` | 是否启用嵌入式 Worker | `true`（默认启用） |
| `ARQ_REDIS_URL` | ARQ 使用的 Redis 地址 | `$CACHE_REDIS_URL` |
| `ARQ_QUEUE_NAME` | 队列名称 | `arq:queue` |
| `ARQ_DEBOUNCE_SECONDS` | 去重时间窗口（秒） | `5` |
| `ARQ_MAX_JOBS` | Worker 最大并发任务数 | `10` |
| `ARQ_POLL_DELAY` | 队列轮询间隔（秒） | `0.5` |
...
| CACHE_BACKEND | 缓存后端 (memory/redis) | memory |
| CACHE_REDIS_URL | Redis 缓存连接地址 | redis://localhost:6379/0 |
| COORDINATOR_URL | 分布式协调后端 (tooz) | (空，单节点模式) |
| ARQ_EMBEDDED_WORKER | 启用嵌入式 ARQ Worker | true（默认启用） |
| ARQ_REDIS_URL | ARQ 任务队列 Redis 地址 | $CACHE_REDIS_URL |
| LOG_LEVEL | 日志级别 | INFO |
| LOG_DIR | 日志文件目录 | (空，仅输出到 stderr) |
